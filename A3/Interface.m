...
if symmetrical
    ...
    % Continue with the next ply layers
...
ply_angle_a2 = 35;  % Example ply angle
...
% TODO: Condition of asym
% in mm so that MPa * mm = N/mm
% laminate_height = 2 * (layer_num * h_o.value + z_c) * 1000;
...
% TODO: This should be an input to the entire program
off_axis_stress_resultant_vector = [8800000; -4800000; 8400000];

off_axis_strain_vector = a_matrix * off_axis_stress_resultant_vector;

% Print the results for on-axis stress and strain vectors
fprintf("\nOff-Axis Stress Resultant Vector (Given) [N/m]:\n");
fprintf('%15.3e\n', off_axis_stress_resultant_vector);
...
for i = 1:length(schedule)
    ...
    on_axis_strain_vector_a3 = transformStrain(off_axis_strain_vector, schedule(i));

    on_axis_stress_vector_a3 = Q * on_axis_strain_vector_a3;
    
    % Print the results for on-axis stress and strain vectors
    fprintf("\nPly Angle: %d degrees\n", schedule(i));
    
    fprintf("\nOn-Axis Stress Vector [MPa]:\n");
    fprintf('%15.3e\n', on_axis_stress_vector_a3);

    fprintf("\nOn-Axis Strain Vector:\n");
    fprintf('%15.3e\n', on_axis_strain_vector_a3);
end